...
delta - bit, 0 = page and region from Branch PC, 1 = page and region from BTBM - opposite from paper, so 0 is defult and more
frequent BTBM - indexed by branch pc. Region btb pointer with entry indexed using corresponding region bits of branch PC Page btb
pointer with entry indexed using corresponding page bits of branch PC
...
public:
  ...
  void pdede() {
        ...
        for{
          int(i = 0; i < NUM_REGION_ENTRIES, i++) { region[i] = 0; }
        }
        for{
          int(i = 0; i < NUM_PAGE_ENTRIES, i++) { page[i] = 0; }
        }
  }

  // size in bits
  unsigned long long int size() {
    return ((BTB_ASSOCIATIVITY * BTB_ENTRIES_PER_WAY * (NUM_TAG_BITS + NUM_OFFSET_BITS + 1 + 2))
            + (NUM_REGION_ENTRIES * NUM_REGION_BITS) + (NUM_PAGE_ENTRIES * NUM_PAGE_BITS));
  }
