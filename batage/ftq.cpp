...
//ftq_entry ftq[NUM_FTQ_ENTRIES];
  ...
  ftq_entry::ftq_entry(const bool &predDir1, const bool& highconf1, const bool &resolveDir1, const uint64_t &pc1,
            const insn_t &insn1, const uint64_t &branchTarget1, const uint64_t &fetch_pc, const batage* bp)
...
void allocate_ftq_entry(AddrType branch_PC, AddrType branch_target,
                        IMLI &IMLI_inst)
#elif defined BATAGE
void ftq::allocate_ftq_entry(const bool &predDir, const bool &highconf, const bool &resolveDir,
...
#endif
{
  ...
  ftq_entry f{predDir, highconf, resolveDir, pc, insn, branchTarget, fetch_pc, bp};
  // Move assignment
...
bool ftq::get_predDir_from_ftq (uint16_t index)
{
	return mem[index].predDir;
}
