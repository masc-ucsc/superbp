...
bool dualcounter::is_counter_reset() { return ((n[0] == 0) && (n[1] == 0)); }
void dualcounter::reset() { n[0] = n[1] = 0; }

int dualcounter::pred() { return (n[1] > n[0]); }

bool dualcounter::mediumconf() { return (n[1] == (2 * n[0] + 1)) || (n[0] == (2 * n[1] + 1)); }

bool dualcounter::lowconf() { return (n[1] < (2 * n[0] + 1)) && (n[0] < (2 * n[1] + 1)); }
...
void path_history::init(int hlen) {
  ...
  hlength = hlen;
printf ("using arrays for histories \n");
  h       = new unsigned[hlen];
...
void SBP_folded_history::init(int original_length, int compressed_length, int injected_bits) {
...
int histories::ghg(int i) const {  // int SBP_NUMG = bp->SBP_NUMG;
...
printf ("using arrays for g and gi \n");
  gi = new int[SBP_NUMG];
...
// fprintf (stderr, "getgo, %d, %d \n", i, offset_within_entry);
  ...
  #else
  */
  // Works with BANK_INTERLEAVING, this particulaar function already sends both the bank and index based on bank interleaving, so no
  // need to do bank[i]
  assert(((offset_within_entry >= 0) && (offset_within_entry < INFO_PER_ENTRY[i])));
  return g[i][gi[i]][offset_within_entry];
  // #endif
...
/*
To add POS - with single tag - pos to offset_within_entry mapping is not constantand may chnage, so each subentry needs to be
...
if (!POS) {
  ...
  for (offset_within_packet = 0; offset_within_packet < FETCHWIDTH; offset_within_packet++) {
    ...
    // b_bi[offset_within_packet], b2_bi2[offset_within_packet]);
#endif  // DEBUG
  }
...
uint32_t batage::get_allocs(int table) { return (allocs[table]); }

/*For superscalar - anything that is non constant & is used before/ without
 assigning a value must be saved in ftq*/
/*
To add POS - with single tag - pos to offset_within_entry mapping is not constantand may change, so each subentry needs to be
checked For update, pos must either be matched again or taken from FTQ, if matched, then consider it a match and proceed as usual
...
// b_bi[offset_within_entry], b2_bi2[offset_within_entry]);
#endif  // DEBUG
...
if (POS || MT_PLUS) {
  ...
  if ((bp[offset_within_packet] < (int)hit[offset_within_packet].size())
      ...
      && s[offset_within_packet][bp[offset_within_packet] + 1].highconf()
...
if (POS || MT_PLUS) {
...
#ifdef DEBUG
  fprintf(stderr,
          "For allocation - s[%d][%d] = %d, %d\n",
          offset_within_packet,
          bp[offset_within_packet],
          s[offset_within_packet][bp[offset_within_packet]].n[0],
          s[offset_within_packet][bp[offset_within_packet]].n[1]);
#endif  // DEBUG
  ...
  if (allocate && ((int)(rando() % MINAP) >= ((cat * MINAP) / (CATMAX + 1)))) {
    ...
    If any bank had hit, i starts = hit[offset_within_packet][0], leftmost hitting table (smallest table number), then we subtract
    randomly a positive number to possibly move to left, so smaller i and then we decrement i in while loop, so inside the while
    loop, none of the tables ever hit. Hence, this will update ALL (but none of thenm hit) till it can ALLOCATE ON ONE, after
...
int batage::size() {
  ...
  for (uint32_t i = 0; i < SBP_NUMG; i++) {
    fprintf(stderr,
            ...
            "%d, SBP_LOGGE = %d\n",
            i,
            ORIG_ENTRIES_PER_TABLE[i],
            SBP_LOGG[i],
            SS_ENTRIES_PER_TABLE[i],
            LOGGE_ORIG[i],
            ENTRIES_PER_TABLE[i],
            SBP_LOGGE[i]);
