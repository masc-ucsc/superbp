...
// static uint16_t index_from_aligned_fetch_pc, last_index_from_aligned_fetch_pc; // starts from pc - (alignedPC) after every
// redirect
/*
...
void branchprof::branchprof_exit(PREDICTOR* bp) {
  ...
  fprintf(pc_trace,
          ...
          /*instruction_count,*/ correct_prediction_count,
          misprediction_count,
  ...
  fprintf(
      ...
      ((double)(num_gshare_predictions - num_gshare_correct_predictions) * 100 / num_gshare_predictions),
...
#ifdef GSHARE_TRACE
  fprintf(gshare_trace,
          "fetchPC = %llx predicted from index = %u with tag = %x and hit = %u\n",
          fetch_pc,
          gshare_index,
          gshare_tag,
          gshare_pred_inst.hit);
#endif
#endif  // GSHARE
...
void branchprof::fetchBoundaryEnd_dromajo() {
  ...
  for (int i = 0; i < (partial_pop ? inst_index_in_fetch : FETCH_WIDTH); i++) {
    if (!(bp->ftq_inst.is_ftq_empty())) {
      ...
      // TODO Check
      if (((update_insn == insn_t::branch) && update_resolveDir && update_predDir && update_highconf)
          || (update_insn == insn_t::jump) /* || (update_insn == insn_t::ret) */) {
        highconfT_in_packet = true;
        if (!gshare_tracking)  // The first must be a branch
        {
          if (!last_gshare_pred_inst.tag_match && !gshare_pred_inst.tag_match
...
#ifdef GSHARE_TRACE
            fprintf(gshare_trace,
                    "fetchPC = %llx allocated at index = %u with tag = %x \n",
                    gshare_PCs[0],
                    update_gshare_index,
                    update_gshare_tag);
#endif
...
#ifdef DEBUG_HUQ
        fprintf(stderr,
                "Allocate huq entry for pc = %llx, target = %llx, resolvedir = %d \n",
                update_pc,
                update_branchTarget,
                update_resolveDir);
#endif
        bp->huq_inst.allocate_huq_entry(/*update_pc,*/ update_branchTarget, update_resolveDir);
      }
...
3. Check if it is sequentially next PC
        If instruction == first in packet (either index == 0 or coming in from a branch/jump etc), then get prediction(s) using
FetchPC- should return (fetch_width * num_subentries) predictions and related info. All must be saved into FTQ. On subsequent calls,
check if PC is within fetch_packet and if it is sequential, if both, then If PC is not sequential i.e. taken branch, then
*/
...
/*
gshare update -
the pc, pos and target info is available int he prediction_packet, in dromajo, we have only one such packet ata a time, in desesc,
we will have multiple in ftq, but only 1 active - dequeued At each instruction resolution - compare pc and pos to see if there was a
gshare prediction, if yes - 1, right or wrong - update the flag when the packet is over and update for the entire packet needs to be
done, gshare nust be updated based on the various resolved flags
...
// fprintf (stderr, "aligned_fetch_pc = %llu, index = %llu and ", aligned_fetch_pc, index_from_aligned_fetch_pc);
  ...
  Two options -
  1. Probably harder - Call get_prediciton multiple times, "all calls to sub entries in same entry must match the same tag". No
  change to ftq.
  2. Probably easier - Call get_prediction only once, it must return all (info per entry) predictions. FTQ must save the following
  info "correct and separate" for each of the pcs in every cc - bp + Check counters "s", bi, bi2, gi, b_bi, b2_bi2
  */
...
bool branchprof::handle_insn_desesc(uint64_t pc, uint64_t branchTarget, uint8_t insn_type, bool taken) {
  ...
  misprediction = true; 
  resolveDir    = taken;
  { bp->ftq_inst.ftq_update_resolvedinfo(inst_index_in_fetch, pc, insn, resolveDir, branchTarget); }
